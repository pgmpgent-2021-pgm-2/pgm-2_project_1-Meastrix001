  ...
  const app = {
   ...
   async getCovidCasesfromAPI() {
    ...
    async searchFunction() {
      const YoutubeAPI = new YTSearchApi()
      const githubAPI = new GitHubAPI();
      const $youtubesearch = document.getElementById('YTSearchResult')
      const $youtubebutton = document.getElementById('youtubeSearch')
      ...
      $youtubesearch.addEventListener('submit', async (evt) => {
      evt.preventDefault();
            if ($youtubebutton.value.length) {
              const YtJson = await YoutubeAPI.searchTroughyoutube($youtubebutton.value)
        const youtube = YtJson.items.map(vid => {
          console.log(vid)
          console.log(vid.snippet.channelTitle)
          console.log(vid.id.videoId)
          return vid
        })
        this.updateYoutubeResults(youtube);
      }
      })
    },
    updateYoutubeResults(data){
      let tempStr = '';
      data.map(video => {
      tempStr += `<li class="youtube_link" data-id="${video.etag}">
      <img src="${video.snippet.thumbnails.default.url}">
      <p>${video.snippet.channelTitle}<p>
      </li>`
      return this.$gitSearchResults.innerHTML = tempStr
      })
      let $userinfo = document.querySelectorAll('.youtube_link')
      $userinfo.forEach(user => {
        user.addEventListener('click', (evt) => {
          this.YTDatasetId = evt.target.dataset.id || evt.target.parentNode.dataset.id
          this.createHTMLForYoutubeVideo(data);
        })
      })
    },
    createHTMLForYoutubeVideo(data){
      data.map(video => {
        if (this.YTDatasetId == video.etag) {
          months = [
            "January",
            "February",
            "March",
          ...
          </p>Geupload op: ${day} ${months[month]} ${year}</p>
          <section class="yt_details">
